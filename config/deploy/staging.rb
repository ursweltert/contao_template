...
set :user, 'user'
set :server, 'server.uberspace.de'

role :web, "#{fetch :user}@#{fetch :user}.#{fetch :server}:22"
role :app, "#{fetch :user}@#{fetch :user}.#{fetch :server}:22"
role :db,  "#{fetch :user}@#{fetch :user}.#{fetch :server}:22", primary: true
...
set :deploy_to,   -> { "/var/www/virtual/#{fetch :user}/capistrano/#{fetch :stage}/#{fetch :application}" }
...
set :db_credentials_file, -> { "/home/#{fetch :user}/.my.cnf.yml"}

# Where can we find root credentials ?
# NOTE: Only required for db_create_user
set :db_root_credentials_file, -> { "/home/#{fetch :user}/.my.cnf.yml"}
...
set :web_conf_file, -> { "/home/#{fetch :user}/nginx/#{fetch :stage}/#{fetch :application}.conf" }

# Which port does the server runs on ?
# Default: 80
set :web_server_listen_port, 47256

# What is the application url ?
# THis is used for Virtual Hosts
set :application_url, %W{#{fetch :user}.#{fetch :server}}
